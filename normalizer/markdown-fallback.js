...
function markdownFallbackReport(rawOutput, fallback = {}) {
  ...
  const report = {
    ...
    qualityGate: { status: "needs_review", missingCriticalFields: ["policy maximum", "pre-existing condition / acute onset wording", "claim notice / proof-of-loss deadline"], manualReviewReasons: ["Unstructured model output required fallback parsing."] },
    missingInformation: [],
    nextSteps: [],
    citations: [],
    answers: [{
